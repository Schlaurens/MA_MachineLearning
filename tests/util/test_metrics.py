...
class TestGetThresholdingMask:
    def testBaseCaseLogicalAnd(self):
        ...
        results = u_metrics.get_thresholding_mask(
            classifier_preds, classifier_threshold, encoder_preds, encoder_threshold, "logical_and"
        )

        assert tf.reduce_all(expected == results)

    def testBaseCaseAdditive(self):
        classifier_threshold = 0.8
        encoder_threshold = 0.1
        ...
        expected = tf.cast(tf.constant([[1, 1, 1], [1, 1, 0]]), tf.bool)
        results = u_metrics.get_thresholding_mask(
            classifier_preds, classifier_threshold, encoder_preds, encoder_threshold, "additive"
        )

        assert tf.reduce_all(expected == results)

    def testFlatInputLogicalAnd(self):
        classifier_threshold = 0.8
        encoder_threshold = 0.1
    ...
    def testFlatInputAdditive(self):
        classifier_threshold = 0.8
        encoder_threshold = 0.1
    ...
    def testHighDimensionsLogicalAnd(self):
        classifier_threshold = 0.8
        encoder_threshold = 0.1
    ...
    def testHighDimensionsAdditive(self):
        classifier_threshold = 0.8
        encoder_threshold = 0.1
